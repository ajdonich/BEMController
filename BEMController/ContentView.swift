...
//  BEMController
//
//  Created by Alfred J Donich on 8/21/24.
//
...
struct ContentView: View {
    ...
    private var bleCtrl = BLEController()
                ...
                ZStack {
                    ...
                    .onChange(of: mHzFreqs[0]) {     // onSliderDragging
                        if mHzAcks_a[0] != mHzFreqs[0] {
                            bleCtrl.writeFrequency(mHz: mHzFreqs[0])
                            freqColors[0] = .orange
                        }
                    }
...
//                    .foregroundColor(ipcCtrl.status ? .green : .red)
            ...
            .task {
                bleCtrl.bindAckRsp(to: $mHzAcks_a)
            }
            .onReceive(hbTimer) { _ in    // heartbeat timer
                bleCtrl.readFrequency()
            }
            .onChange(of: mHzAcks_a) {    // heartbeat ack
                ...
                for i in 0..<mHzAcks_a.count {
                    ...
                    else if i == 0 { ackmatch = false }
                }
                if !ackmatch { bleCtrl.writeFrequency(mHz: mHzFreqs[0]) }
            }
        }
    }
}
